...
function ImageStory() {
  ...
  const { remainingTests: initialRemainingTests = 0, testKey = 'image_story' } = location.state || {};
  ...
  const [activeTab, setActiveTab] = useState('ImageStory Dashboard');
    ...
    const fetchTestCounts = async () => {
      ...
      if (storedEmail) {
        try {
          ...
          setRemainingTests(parsedData.tests?.image_story || 0);
        } catch (error) {
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      const promptResponse = await fetch('https://au03f6dark.execute-api.ap-south-1.amazonaws.com/dev/prompt-gen-ai-story', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
          ...
          <div className="test-container">
            <div className="card" style={{
              ...
              <div style={{ display: 'flex', gap: 30 }}>
                <div style={{ flex: 1 }}>
                  <h2 style={{ color: 'var(--accent)', marginBottom: 16 }}>🚀 ImageStory Test in Progress</h2>
                  <p style={{ fontSize: 14, marginBottom: 20, color: 'var(--muted)' }}>
                    Observe the image carefully and start telling a story based on what you see. You have one chance to record your description for up to 50 seconds.
                  </p>
