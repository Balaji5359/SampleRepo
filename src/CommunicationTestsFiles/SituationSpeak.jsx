...
export default function SituationSpeak({
    ...
    const { remainingTests: initialRemainingTests = 0, testKey = 'situation_test' } = location.state || {};
    ...
    const [sessionId] = useState(`situation-speak-test-${String(Math.floor(Math.random() * 10000000)).padStart(7, '0')}`);
        ...
        const fetchTestCounts = async () => {
            ...
            if (storedEmail) {
                try {
                    ...
                    setRemainingTests(parsedData.tests?.situation_test || 0);
                } catch (error) {
            ...
            sessions.forEach(session => {
                ...
                session.conversationHistory?.forEach(conv => {
                    if (conv.agent) {
                        // DETAILED SCORES:
                        // - JAM Score: 5.6 / 10
                        const scoreMatch = conv.agent.match(/JAM Score:\s*([\d.]+)\s*\/\s*10/i);
                        const wordsMatch = conv.agent.match(/WORD COUN: (\d+) words/i);
                        ...
                        if (wordsMatch) {
                            sessionWords = parseInt(wordsMatch[1]);
                ...
                if (sessionScore > 0 && sessionWords > 0) {
                    chartData.push({
                        session: session.sessionId.replace('situation-speak-test-', ''),
                        score: sessionScore,
                        words: sessionWords,
                                    ...
                                    <div style={{ display: 'flex', gap: 30 }}>
                                        <div style={{ flex: 1 }}>
                                            <h2 style={{
                                                ...
                                                {theme === 'light' ? '🎯 Speaking Challenge' :
                                                    theme === 'custom' ? '✨ Excellence Mode' :
                                            ...
                                            <p style={{
                                                ...
                                                {theme === 'light' ? 'Showcase your communication skills in this focused speaking session.' :
                                                    theme === 'custom' ? 'Elevate your speaking prowess in this premium experience.' :
