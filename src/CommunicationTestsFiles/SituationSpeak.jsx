...
export default function SituationSpeak({
  ...
  const [sessionId, setSessionId] = useState(`situation-test-${Math.floor(Math.random() * 10000000)}`);
  ...
  const sendMessageToSituation = async (message) => {
    setIsLoading(true);
    try {
      const requestBody = {
        body: {
      ...
      const response = await fetch('https://ibxdsy0e40.execute-api.ap-south-1.amazonaws.com/dev/situationagent-test', {
        method: 'POST',
  ...
  const extractButtons = (text) => {
    const buttons = [];
    if (text.includes('Type "start"') || text.includes('type "start"') || text.includes('Type \'start\'') || text.includes('type \'start\'')) {
      buttons.push('start');
    }
  ...
  const handleButtonClick = (buttonText) => {
    sendMessageToSituation(buttonText);
  };
  ...
  const startTest = async () => {
    setSessionId(`situation-test-${Math.floor(Math.random() * 10000000)}`);
    setChatMessages([]);
    ...
    setTimeLeft(120);
    setCurrentRecording('');
    await sendMessageToSituation('hi');
  ...
  const startMicRecording = () => {
    ...
    recognition.onresult = (event) => {
      ...
      if (event.results[event.results.length - 1].isFinal) {
        sendMessageToSituation(transcript);
        stopMicRecording();
      }
    };
  ...
  return (
    <div className="jam-root" role="application" aria-label="SituationSpeak Dashboard">
      <div className="jam-topnav" role="navigation" aria-label="Top navigation">
        <div className="left">
          <div className="jam-title" aria-hidden>
            SituationSpeak Practice
          </div>
          <div className="jam-nav" role="tablist" aria-label="Main tabs">
            {['Back', 'Practice', 'SituationSpeak Dashboard', 'SituationSpeak Leaderboard'].map((t) => (
              <button
                key={t}
                role="tab"
          ...
          <div className="jam-left">
            {!showTestPopup && (
              <div className="card" style={{ textAlign:'center', padding:60 }}>
                <div style={{ fontWeight:700, fontSize:32, marginBottom:20, color:'var(--accent)' }}>SituationSpeak Practice Test</div>
                <div style={{ fontSize:18, color:'var(--muted)', marginBottom:40 }}>Test your situational speaking skills</div>
                
                <button className="start-btn" onClick={startTest}>
                  Start SituationSpeak Test
                </button>
            ...
            <div className="card performance-card" style={{ minHeight:320 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                <div>
                  <div style={{ fontWeight:700, color:'var(--text-color)' }}>
                    {theme === 'custom' ? 'Speaking Analytics' : theme === 'light' ? 'Progress Tracking' : 'Performance Over Time'}
                  </div>
                  <div style={{ fontSize:13, color:'var(--muted)' }}>
                    {theme === 'custom' ? 'Communication metrics — recent activity' : theme === 'light' ? 'Your improvement journey' : 'Score & Words — last 12 sessions'}
                  </div>
                </div>
                <div style={{ fontSize:13, color:'var(--muted)' }}>
                  <small>{theme === 'custom' ? 'Live Data' : theme === 'light' ? 'Updated' : 'Interactive'}</small>
                </div>
              </div>
              ...
              <div className="chart-area performance-card" style={{ padding:8 }}>
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)' }}>
                  {theme === 'custom' ? 'Analytics dashboard loading...' : theme === 'light' ? 'Progress chart will appear after tests' : 'Chart will appear after taking tests'}
                </div>
              </div>
            </div>
          </div>
              ...
              <div style={{ display:'flex', gap:30 }}>
                <div style={{ flex:1 }}>
                  <h2 style={{ color:'var(--accent)', marginBottom:16 }}>
                    {theme === 'custom' ? 'Live Communication Assessment' : theme === 'light' ? 'Speaking Skills Evaluation' : 'SituationSpeak Test in Progress'}
                  </h2>
                ...
                <div style={{ flex:1, textAlign:'center' }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:16, marginBottom:20 }}>
                    ...
                    <div style={{ fontSize:'14px', color:'var(--muted)' }}>
                      {micRecording ? 'Recording...' : 'Click to speak'}
                    </div>
                  </div>
