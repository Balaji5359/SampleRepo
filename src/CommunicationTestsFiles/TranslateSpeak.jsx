...
export default function TranslateSpeak({
    ...
    const { remainingTests: initialRemainingTests = 0, testKey = 'translate_test' } = location.state || {};
    ...
    const [sessionId] = useState(`translate-speak-test-${String(Math.floor(Math.random() * 10000000)).padStart(7, '0')}`);
        ...
        const fetchTestCounts = async () => {
            ...
            if (storedEmail) {
                try {
                    ...
                    setRemainingTests(parsedData.tests?.translate_test || 0);
                } catch (error) {
        ...
        try {
            ...
            sessions.forEach(session => {
                ...
                session.conversationHistory?.forEach(conv => {
                    if (conv.agent) {
                        const scoreMatch = conv.agent.match(/Final Overall Score:\s*(\d+)\//i);
                        const wordsMatch = conv.agent.match(/WORD COUNT: (\d+) words/i);
                ...
                if (sessionScore > 0 && sessionWords > 0) {
                    chartData.push({
                        session: session.sessionId.replace('translate-speak-test-', ''),
                        score: sessionScore,
                        words: sessionWords,
                                ...
                                <div className="jam-left">
                                    {!showTestPopup && (
                                        <div className="card" style={{ textAlign: 'center', padding: 60 }}>
                                            <div style={{ fontWeight: 700, fontSize: 32, marginBottom: 20, color: 'var(--accent)' }}>Translate-Speak Test</div>
                                            ...
                                            <button className="start-btn" onClick={startTest} disabled={remainingTests <= 0}>
                                                {remainingTests <= 0 ? 'No Tests Remaining' : 'Start Translate & Speak Test'}
                                            </button>
